...
export default function Navbar() {
  ...
  return (
    ...
    >
      {userToken && (
        <CreatePostButton
          clasN={`fixed bottom-16 left-10 md:bottom-12   ${location.pathname !== "/" ? "hidden" : "block"} `}
        />
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={toggleTheme}
          className={
            "dark:text-background bg-background text-primaryText z-50 flex h-13 w-13 cursor-pointer items-center justify-center rounded-full border border-[#791f4c2a] font-semibold transition-all duration-300 hover:scale-110 hover:opacity-90 dark:border-0 dark:bg-[#161515]" +
            (scroll
              ...
              <div className="dark:text-background absolute right-0 z-10 mt-2 flex w-72 flex-col gap-1 overflow-hidden rounded-md shadow-xl dark:bg-[#1b1a1a]">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n: Notification) => (
                    <div
                      key={n.id}
                      className={`px-5 py-3 transition ${!n.isRead ? "bg-gray-100 dark:bg-[#0c0a0c]" : "bg-inherit"} text-md hover:bg-[#c911391a] dark:hover:bg-gray-900`}
                    >
                      {n.type === "FOLLOW" ? (
                        <Link
